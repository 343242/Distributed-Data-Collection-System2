...
`CollectorTask` 是最小控制单元，表示“一个数据源引用 + 一套采集配置 + 一个 ConnectorType + 一个任务配置版本”。任务属于 `Customer`，通过 `assignedAgentId` 分配给某个 Agent，而不是 Agent 的强子对象。
...
- `LocalBuffer`：负责可靠写入、批次状态、索引、游标、修复和清理。
...
具体数值属于容量规划决策，不能在缺少业务吞吐目标时臆定。进入详细设计或研发拆分前必须补齐。
...
- `AppendLog` 是事实来源，`BufferIndex` 必须可通过 `RepairScanner` 重建。
- 记录写入必须先保证 AppendLog 持久化，再提交索引状态。
- `messageId` 序列、记录索引、Batch 状态和 checkpointCandidate 必须通过可恢复的原子更新机制维护。
- 如果使用 RocksDB / Pebble，索引、游标、序列和 Checkpoint 更新应使用原子 batch 写入。
- Agent 重启时，`UPLOADING` 批次必须回退为 `PENDING` 或 `RETRY_WAIT`，不得假定已成功。
- Index 已写但 AppendLog 不完整时，以 AppendLog 为准修复或丢弃损坏索引项。
- AppendLog 已写但 Index 缺失时，应通过扫描重建 Index。

LocalBuffer 隔离策略：

- 第一版至少保证任务级逻辑隔离，每个任务有独立命名空间、独立 quota、独立状态和独立清理边界。
- 物理上可以共享一个嵌入式存储实例，也可以每任务独立实例；该选择属于详细设计阶段的资源与故障隔离权衡。
...
具体配额数值必须结合业务规模和部署资源确认。文档不臆定默认容量。
...
- 端到端延迟目标。
- LocalBuffer 最大磁盘占用。
- 可承受 Gateway 不可用的最小时长。
- 最老未 ACK 数据告警阈值。

这些数值需要业务规模、部署环境和硬件资源输入，本文不臆定。

## 25. 原始需求追溯矩阵

| 原始需求 | 设计章节 | 设计响应 |
|----------|----------|----------|
| 1. 客户 Agent 是客户现场独立服务器，同一客户可能多个 Agent | 第 2、3、5、20 节 | 引入 Customer 作为边界，Agent 归属 Customer，支持多个 Agent 并通过注册凭证纳管。 |
| 2. 一个 Agent 上根据配置运行多个 Connector | 第 5、7、8、21 节 | Gateway 创建 CollectorTask，Agent 根据期望状态和版本收敛为 ConnectorInstance。 |
| 3. Connector 有不同类型，不同类型是不同程序包，同一程序包可启动多个实例 | 第 3、21 节 | 使用 ConnectorType、ConnectorPackageVersion、TaskConfigVersion 建模，同一包可服务多个任务实例。 |
| 4. Agent 负责 Connector 创建、停止、监控等管理 | 第 5、6、7、8、19 节 | Agent 管理实例生命周期、本地提交、状态上报、日志和资源限制。 |
| 5. 采集数据进入数据存储 | 第 2、9、10、11、13、14 节 | 数据链路为 ConnectorInstance -> Agent LocalBuffer -> Gateway Ingest -> 数据存储，并定义 Envelope、Batch、ACK 和幂等。 |
| 6. 数据网关负责控制、程序包下发、监控、启停通知 | 第 4、8、19、21 节 | Gateway 控制面负责注册发现、任务编排、程序包管理、启停控制、监控告警和审计。 |
| 7. 异常情况下保证稳定性，最大可能保证数据不丢 | 第 9、12、13、14、15、16、17 节 | 至少一次语义、本地可靠缓冲、未 ACK 重传、幂等写入、ACK 后推进 Checkpoint、背压和网络分区恢复。 |
| 8. 根据负载动态分发，注意数据完整性 | 第 8、23 节 | 第一版不自动动态分发，但实际使用 assignmentVersion，并预留 migrationState、lastConfirmedCheckpoint 和迁移模式。 |

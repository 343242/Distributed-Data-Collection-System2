...
```text
messageId = agentId + storageStreamId + writeSeq
...
- `storageStreamId` 是 Agent 本地稳定标识，用于区分同一 Agent 内不同任务或不同本地缓存流。
...
- 采集数据记录或数据块。
- 记录级唯一键或等价唯一约束：`customerId + taskId + agentId + storageStreamId + writeSeq`。
- 批次 / 区间级提交记录：`customerId + taskId + agentId + storageStreamId + firstWriteSeq + lastWriteSeq + recordCount + payloadDigest + checkpointCandidateDigest`。
- 提交状态：`COMMITTED`。
